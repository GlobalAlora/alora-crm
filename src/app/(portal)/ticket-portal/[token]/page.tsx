...
export default function TicketTrackingPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const reactToComment = useMutation({
    mutationFn: ({ commentId, emoji }: { commentId: string; emoji: string }) =>
      fetch(`/api/portal/tickets/${token}/comments/${commentId}/react`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ emoji }),
      }).then(r => r.json()),
    onMutate: async ({ commentId, emoji }) => {
      await qc.cancelQueries({ queryKey: ['portal-ticket', token] })
      const prev = qc.getQueryData<{ data: PortalTicket }>(['portal-ticket', token])
      if (prev?.data) {
        qc.setQueryData(['portal-ticket', token], {
          ...prev,
          data: {
            ...prev.data,
    ...
    onError: (_err, _vars, ctx) => {
      if (ctx?.prev) qc.setQueryData(['portal-ticket', token], ctx.prev)
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: ['portal-ticket', token] })
    },
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  ...
                  {ticket.comments.map(c => (
                    <div
                      key={c.id}
                      className={`group px-6 py-4 ${c.is_client ? 'bg-blue-50/50 dark:bg-blue-950/20' : ''}`}
                    >
                      ...
                      {/* Reactions */}
                      {!isAdminPreview && (
                        <div className="flex items-center gap-1 flex-wrap mt-2 pl-8">
                          {REACTION_EMOJIS.map(emoji => {
                            const users = (c.reactions ?? {})[emoji] ?? []
                            if (!users.length) return null
                            const iReacted = users.includes(CLIENT_ID)
                            return (
                              <button
                                key={emoji}
                                ...
                                className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs border transition-colors ${
                                  iReacted
                                    ...
                                    : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-blue-300 hover:text-blue-500'
                                }`}
                              >
                                <span>{emoji}</span>
                          ...
                          <span className={`relative transition-opacity ${emojiPickerForComment === c.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                            <button
                              ...
                              className="inline-flex items-center justify-center w-6 h-5 rounded-full text-xs bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 hover:border-blue-300 hover:text-blue-500 transition-colors"
                            >
                              +
                            </button>
                            {emojiPickerForComment === c.id && (
                              <div className="absolute bottom-full left-0 mb-1 flex items-center gap-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-2 py-1.5 shadow-lg z-10">
                                {REACTION_EMOJIS.map(emoji => (
                                  <button
                                    key={emoji}
