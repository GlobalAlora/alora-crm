...
- **Tecnología y stack**: recomendá una tecnología o plataforma CONCRETA y real (ej. "WordPress + WooCommerce", "Shopify", "Next.js con backend headless", "n8n autoalojado") — no una descripción vaga tipo "una solución robusta y escalable". Justificá la elección en 2-3 frases atadas a lo que dijiste en el contexto (por qué esta plataforma resuelve lo que el cliente necesita y no otra), no una lista de checkboxes genéricos de "qué se va a evaluar". Si de verdad hay más de una opción viable, mencioná la alternativa considerada y por qué se descartó — igual que en el contexto. Esto es sobre la tecnología del PROYECTO DEL CLIENTE, que sí se nombra explícitamente en toda propuesta real; no confundir con la regla de abajo sobre el stack interno de Alora.
